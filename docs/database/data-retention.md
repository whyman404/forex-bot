...
## 13. Phase-2.5 / Round-4 additions (2026-06-16) — ``tv_signal``

### 13.1 New strategy: external-signal provider

Migration 0005 introduces the ``tv_signal`` catalog row + two columns:
- ``strategies.requires_external_service`` BOOL
- ``strategy_instances.external_signal_provider`` VARCHAR

### 13.2 Retention — no special treatment

Signals generated by ``tv_signal`` strategies follow the **same 18-month
online retention** as signals from all other strategies. They share the
``signals_y<year>_m<month>`` partitions and are pruned by
``maintain_partitions.py`` on the same schedule. There is no need for a
tv-specific partition or retention rule.

Trades derived from TV signals follow the same **7-year regulatory
retention** as any other trade on ``trades``.

### 13.3 GDPR export

``signals.reason`` for TV-sourced signals contains the multi-timeframe
recommendation breakdown (ticker + score + per-interval breakdown). This
data is **non-PII** (public market data) but it IS user-attributable via
``signals.strategy_instance_id -> users.id``. Therefore:

- The DSAR export (``GET /me/export`` → ``account_exports``) MUST
  include the ``signals.reason`` JSONB for the user's instances.
- The export MUST also include ``strategy_instances.external_signal_provider``
  so the user knows TradingView influenced their trades.

See ``docs/database/tv_signal-data-model.md`` §7 for the field-level
classification.

### 13.4 Purge job — no change required

Because ``signals.reason`` for TV-sourced rows contains no PII, the
existing purge logic in ``scripts/purge_soft_deleted.py`` covers this
case without modification. The FK ``signals.strategy_instance_id ->
strategy_instances.user_id`` already de-identifies on user purge (the
user row's identifiers are scrubbed but the UUID remains, same pattern
as ``audit_log``).

### 13.5 External-service dependency

``strategies.requires_external_service = TRUE`` rows imply that an
operational outage of the external provider can suspend signal
generation. This is **not a data-retention concern** but is documented
here so the DR runbook owner knows that restoring the DB does not by
itself restore tv_signal functionality — the TradingView credentials /
network reachability must also be verified.
